...
-- no appearance resource installed, still lets the target client open its native adapter's openMenu
function CUSTOM_CLOTHING:giveClothing(playerId, fullCustomization)
    TriggerClientEvent(events.client.clothingOpenMenu, playerId, fullCustomization == true)
    return true
end

function CUSTOM_CLOTHING:saveOutfit(citizenId, name, clothingData)
...
function CUSTOM_CLOTHING:loadOutfit(citizenId, name)
...
function CUSTOM_CLOTHING:getOutfits(citizenId)
...
function CUSTOM_CLOTHING:deleteOutfit(citizenId, name)
    if citizenId == nil or name == nil then
        return false
    end
