...
function ILLENIUM_CLOTHING:giveClothing(playerId, fullCustomization)
    TriggerClientEvent(events.client.clothingOpenMenu, playerId, fullCustomization == true)
    return true
end

function ILLENIUM_CLOTHING:saveOutfit(citizenId, name, clothingData)
...
function ILLENIUM_CLOTHING:loadOutfit(citizenId, name)
...
function ILLENIUM_CLOTHING:getOutfits(citizenId)
...
function ILLENIUM_CLOTHING:deleteOutfit(citizenId, name)
    if citizenId == nil or name == nil then
        return false
    end
